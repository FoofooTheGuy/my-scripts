...
for dir in */; do # iterate seasons of a show
	...
	for ep in "${dir}"/*.*; do # iterate episodes in each dir
		#echo "$ep"
		ffmpeg -sn -hwaccel vaapi -vaapi_device /dev/dri/renderD128 -i "$ep" -map 0 -c copy -c:v h264_vaapi -crf 18 -maxrate 3M -bufsize 3M -pix_fmt yuv420p -vf 'format=nv12,hwupload' "transcoded/$ep [x264].mkv" # hardware accelerated encode
	done
done
